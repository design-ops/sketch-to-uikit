...
layerstyles - generate swift Stylist LayerStyle code from a Sketch file

SYNOPSIS

    ./layerstyles.sh -s sketch-file
...
function run_sketchtool {
    if [[ "${skip_sketchtool}" = true ]] ; then
        ...
        cat "$tmp_dir/document.json" | docker run -i sketch-to-stylable-uikit jq '{layerStyles: .layerStyles, foreignLayerStyles: [.foreignLayerStyles[].localSharedStyle]}' > ${json_file}
        rm -rf "$tmp_dir"
    fi
}
...
function run_docker_styles {
    if [[ "${skip_build}" = true ]] ; then
        ...
        docker_run python src/process_layer_styles.py --input-json ${json_file} --input-sketch ${sketch_file} --output ${output_dir}
